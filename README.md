...
Obsidian vault where **I source and decide, and the active agent does all the writing, cross-linking, and
...
| Path | What it is |
|---|---|
| **`AGENTS.md` / `CLAUDE.md`** | The identical schema for Codex and Claude. Start with the file for the active agent. |
...
| **`wiki/`** | Agent-maintained pages: `strategy/`, `site/`, `pages/`, `tech/`, `reference/`. |
...
- **Ingest** — I drop a source in `raw/` and ask the active agent to process it. The agent reads it, updates the
  relevant pages, the index, and the log.
- **Query** — I ask a question; the active agent answers from the wiki with citations and files good answers
  back as new pages.
- **Lint** — periodically the active agent health-checks the wiki for contradictions, stale claims, orphans,
  and missing links.
